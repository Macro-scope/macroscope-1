...
import { CompactSelection, DataEditor, DataEditorProps, DataEditorRef, GetRowThemeCallback, GridCellKind, GridColumn, GridMouseEventArgs, Theme,  } from "@glideapps/glide-data-grid";
...
import { AlertDialog, AlertDialogContent, AlertDialogDescription, AlertDialogAction, AlertDialogCancel } from "@/components/ui/alert-dialog";
...
const DataTable = ({ mapId }: { mapId: string }) => {
  ...
  const { data, setData, loading, error, addRow, updateRow, deleteRow, reorderRow, 
  ...
  const getCellContent = React.useCallback(([col, row]: readonly [number, number]) => {
...
const getCanvasSize = () => {
  ...
  const onCellEdited = React.useCallback(async ([col, row]: readonly [number, number], newValue: any) => {
    ...
    } else if (column.type === 'multiselect' && newValue.kind === GridCellKind.Custom) {
      const selectedValue = newValue.data.values[0];
      
      const existingOption = column.options?.find(
        (opt: any) => opt.value === selectedValue || opt.label === selectedValue
      );
  ...
  const onRowMoved = useCallback(async (from: number, to: number) => {
...
window.location.reload();
  ...
  const handleSort = useCallback((columnId: string | null, direction?: 'asc' | 'desc') => {
    if (!columnId) {
      setSortConfig({
        column: null,
        direction: null,
      ...
      if (sortConfig.originalOrder) {
        ...
        const sortedData = sortConfig.originalOrder.map(index => originalData[index]);
        setData(sortedData);
      }
      return;
    }
    ...
    setSortConfig(prevConfig => ({
      ...
      originalOrder: prevConfig.originalOrder || data.map((_, index) => index)
    }));
    ...
    setData(prevData => {
      const sortedData = [...prevData].sort((a, b) => {
        ...
        if (columnId === 'description') {
          return (a.description?.markdown ?? '').localeCompare(b.description?.markdown ?? '');
        }
          ...
          <div className="flex items-center gap-1">
            ...
            >
              {isLoading ? (
                ...
                <><Trash2/> {selectedRows.size > 0 ? `(${selectedRows.size})` : ''}</>
              )}
            </Button>
            ...
            <Popover open={sortPopoverOpen} onOpenChange={setSortPopoverOpen}>
              <PopoverTrigger asChild>
                ...
                >
                  ...
                  {sortConfig.column 
                    ? ` ${columns.find(col => col.id === sortConfig.column)?.title}`
                    : ""}
                </Button>
              </PopoverTrigger>
              <PopoverContent className="w-[240px] p-0 bg-white border border-gray-200 shadow-lg">
                ...
                <div className="p-1.5">
                  <select 
                    ...
                    onChange={(e) => setPendingSortConfig(prev => ({
                      ...prev,
                      column: e.target.value || null,
                      direction: e.target.value ? (prev.direction || 'asc') : null
                    }))}
                  ...
                  >
                    ...
                    {columns
                      .filter(col => col.type !== 'button' && col.type !== 'image')
                      .map(column => (
            ...
            <AlertDialog open={showSortWarning} onOpenChange={setShowSortWarning}>
              <AlertDialogContent className="bg-white p-4 max-w-[360px] rounded-lg">
                ...
                <AlertDialogDescription className="text-sm text-gray-600 mb-4">
                  Removing the sorting will set the current order of rows as the default.
                  ...
                  <AlertDialogAction
                    onClick={() => {
                      if (pendingReorder) {
                        ...
                        handleSort(pendingSortConfig.column!, pendingSortConfig.direction as 'asc' | 'desc');
                        setSortPopoverOpen(false);
                      }
            ...
            // rowSelect="multi" 
             rowSelectionBlending='exclusive'
              ...
              onCellClicked={([col, row]) => {
                ...
                if (col < 0 || row < 0 || col >= columns.length || row >= data.length) {
        ...
        <Sheet open={!!selectedRow} onOpenChange={() => setSelectedRow(null)}>
          <SheetContent>
            ...
            onSave={async (updatedData: any) => {
              try {
                if (updatedData.tag_id) {
                  console.log('Updating row with tag_id:', updatedData.tag_id);
                  // First, check if a card already exists for this tag in this map
            ...
            console.log('Existing card:', existingCard);
                  ...
                  // Update the tile with both tag_id and card_id
                  const dbUpdates = {
                    ...updatedData,
                    tag_id: updatedData.tag_id,
                    card_id: cardId,
                  ...
                  await updateRow(selectedRow.id, dbUpdates);
            
                  // Update the card's settings if needed
                  await supabase
                    .from('cards')
                    .update({
                      // Add any card-specific updates here if needed
                    })
                    .eq('card_id', cardId);
            
                } else {
                  // Handle other updates that don't involve category
