...
export const localCardIdSlice = createSlice({
  name: "local-card",
  initialState: { cardId: "", card: null },
  reducers: {
    setLocalCardId: (state, action: PayloadAction<string>) => {
      state.cardId = action.payload;
    },
    setLocalCard: (state, action: PayloadAction<any>) => {
      state.card = action.payload;
    },
  },
});

export const { setLocalCardId, setLocalCard } = localCardIdSlice.actions;
